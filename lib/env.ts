 ...
 *
 * Note: this module validates all vars on load, so it is intentionally NOT
 * imported by the Edge OG route (which only needs GITHUB_TOKEN and reads it
 * directly) — that keeps a malformed unrelated var from affecting that endpoint.
 */
