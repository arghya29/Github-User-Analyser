...
describe("computeHealthScore — recency tiers", () => {
  it("scores 40 when updated within 30 days (boundary: exactly 30)", () => {
    expect(
      computeHealthScore(makeRepo({ updated_at: daysAgo(30) })).breakdown
        .recency,
    ).toBe(40);
  });
  ...
  it("scores 32 when updated between 31 and 90 days (boundary: exactly 90)", () => {
    expect(
      computeHealthScore(makeRepo({ updated_at: daysAgo(90) })).breakdown
        .recency,
    ).toBe(32);
  });
  ...
  it("scores 22 when updated between 91 and 180 days (boundary: exactly 180)", () => {
    expect(
      computeHealthScore(makeRepo({ updated_at: daysAgo(180) })).breakdown
        .recency,
    ).toBe(22);
  });
  ...
  it("scores 12 when updated between 181 and 365 days (boundary: exactly 365)", () => {
    expect(
      computeHealthScore(makeRepo({ updated_at: daysAgo(365) })).breakdown
        .recency,
    ).toBe(12);
  });
  ...
  it("scores 4 when updated more than 365 days ago", () => {
    expect(
      computeHealthScore(makeRepo({ updated_at: daysAgo(400) })).breakdown
        .recency,
    ).toBe(4);
  });
  ...
  it("scores 40 just under the 30-day edge (29 days)", () => {
    expect(
      computeHealthScore(makeRepo({ updated_at: daysAgo(29) })).breakdown
        .recency,
    ).toBe(40);
  });
  ...
  it("drops to 32 just over the 30-day edge (31 days)", () => {
    expect(
      computeHealthScore(makeRepo({ updated_at: daysAgo(31) })).breakdown
        .recency,
    ).toBe(32);
  });
});
...
describe("computeHealthScore — issue health", () => {
  it("returns the neutral-good 24 when no issues have ever been filed", () => {
    const result = computeHealthScore(
      makeRepo({ open_issues_count: 0, closed_issues_count: 0 }),
    );
  ...
  it("scores full 30 when every issue is closed", () => {
    const result = computeHealthScore(
      makeRepo({ open_issues_count: 0, closed_issues_count: 10 }),
    );
  ...
  it("rounds the closed ratio to the nearest point (7 of 10 closed → 21)", () => {
    ...
    const result = computeHealthScore(
      makeRepo({ open_issues_count: 3, closed_issues_count: 7 }),
    );
  ...
  it("rounds a fractional ratio (1 of 3 closed → round(10) = 10)", () => {
    ...
    const result = computeHealthScore(
      makeRepo({ open_issues_count: 2, closed_issues_count: 1 }),
    );
...
describe("computeHealthScore — license and documentation", () => {
  it("awards 15 for license when a license is present", () => {
    expect(
      computeHealthScore(makeRepo({ license: "MIT" })).breakdown.license,
    ).toBe(15);
  });

  it("awards 0 for license when license is null", () => {
    expect(
      computeHealthScore(makeRepo({ license: null })).breakdown.license,
    ).toBe(0);
  });

  it("awards 15 for documentation when a non-empty description is present", () => {
    expect(
      computeHealthScore(makeRepo({ description: "A useful tool" })).breakdown
        .documentation,
    ).toBe(15);
  });
  ...
  it("awards 0 for documentation when the description is empty", () => {
    expect(
      computeHealthScore(makeRepo({ description: "" })).breakdown.documentation,
    ).toBe(0);
  });

  it("awards 0 for documentation when the description is whitespace-only", () => {
    expect(
      computeHealthScore(makeRepo({ description: "   \t  " })).breakdown
        .documentation,
    ).toBe(0);
  });
});
