...
function getMilestoneProgress(value: number, milestones: number[]) {
  ...
  const prevMilestone =
    completed.length > 0 ? completed[completed.length - 1] : 0
  const progress = next
    ? Math.min(
        100,
        Math.round(((value - prevMilestone) / (next - prevMilestone)) * 100)
      )
    : 100
  return {
    ...
    <div className="bg-white dark:bg-slate-700/50 border border-gray-200 dark:border-slate-600 rounded-lg p-6 h-full relative">
      ...
      <div className="space-y-5">
        {achievements.map((item) => {
          const { completedCount, totalCount, next, progress } =
            getMilestoneProgress(item.value, item.milestones)
