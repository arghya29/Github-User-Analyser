...
export default function RepoHealthAnalysisPanel({
  repo,
}: RepoHealthAnalysisPanelProps) {
  const { score, label, breakdown } = computeHealthScore(repo)

  const tips = []
  if (breakdown.recency < 40) {
    tips.push(
      'Repository has not been updated recently. Consider commit releases.'
    )
  }
  ...
  if (breakdown.license === 0) {
    tips.push(
      'Missing license details. Add a LICENSE file (e.g. MIT, Apache-2.0).'
    )
  }
  if (breakdown.documentation === 0) {
    tips.push(
      'Missing description. Update the repository about section or README.'
    )
  }
      ...
      {tips.length > 0 ? (
        <div className="space-y-1.5 mt-2">
          ...
          {tips.map((tip, i) => (
            <div
              key={i}
              className="flex gap-2 text-xs text-gray-600 dark:text-gray-400 items-start"
            >
