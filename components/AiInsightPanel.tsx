...
export default function AiInsightPanel({ user, repos, totalContributions, productivity }: AiInsightPanelProps) {
  ...
  return (
    <div className="bg-white dark:bg-slate-700/50 border border-gray-200 dark:border-slate-600 rounded-lg p-6">
      ...
      {/* New UI Controls for Tone and Length */}
      <div className="flex flex-wrap gap-4 mb-4 items-center">
        <div className="flex items-center gap-2">
          <label htmlFor="tone-select" className="text-xs font-medium text-gray-700 dark:text-gray-300">Tone:</label>
          <select
            id="tone-select"
            value={bioTone}
            onChange={(e) => setBioTone(e.target.value as ToneType)}
            disabled={loading}
            className="text-xs bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-600 rounded-md px-2 py-1.5 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
        ...
        <div className="flex items-center gap-2">
          <label htmlFor="length-select" className="text-xs font-medium text-gray-700 dark:text-gray-300">Length:</label>
          <select
            id="length-select"
            value={bioLength}
