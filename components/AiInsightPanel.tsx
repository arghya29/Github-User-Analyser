...
type InsightType = 'bio' | 'roast' | 'consistency' | 'growth' | 'learning' | 'resume' | 'linkedin' | 'skill-gap'
...
function parseLinkedInResponse(rawText: string) {
  ...
  const headlinesMatch = rawText.match(/---HEADLINES---([\s\S]*?)(?:---SUMMARY---|$)/i)
  ...
  return (
    <div className="bg-white dark:bg-slate-700/50 border border-gray-200 dark:border-slate-600 rounded-lg p-6">
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        ...
        >
          {loading && activeType === 'linkedin' ? 'Optimizing...' : 'LinkedIn Profile'}
        </button>
        <button
      ...
      {text && (() => {
        if (activeType === 'resume' || activeType === 'skill-gap') {
          ...
          const finalItems = items.length > 0 ? items : text.split('\n').map((l) => l.trim()).filter((l) => l.length > 0);

          return (
            <div className="space-y-4">
              <div className="flex justify-between items-center mb-2">
                <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                  {activeType === 'resume' ? 'Recruiter Ready Bullet Points' : 'Complementary Technologies to Learn'}
                </span>
                <button
            ...
            <div className="space-y-5">
              <div>
                <div className="flex justify-between items-center mb-2.5">
                  <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Suggested Headlines</span>
                </div>
              ...
              {parsed.summary && (
                <div>
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">About / Summary</span>
                    <button
                      onClick={() => handleCopyText(parsed.summary, 'linkedin-summary')}
                      className="flex items-center gap-1 text-xs text-cyan-600 dark:text-cyan-400 hover:underline font-medium"
                    >
                      {copiedId === 'linkedin-summary' ? 'Copied Summary!' : 'Copy Summary'}
                    </button>
                  </div>
                  <div className="p-3.5 bg-gray-50 dark:bg-slate-800/80 border border-gray-150 dark:border-slate-700 rounded-lg">
                    <p className="text-gray-700 dark:text-gray-300 text-sm leading-relaxed whitespace-pre-wrap">{parsed.summary}</p>
                  </div>
                </div>
              )}
