...
function createComponents(owner: string, repoName: string): Components {
  return {
    ...
    h2: ({ ...props }) => (
      <h2
        className="text-xl font-bold text-gray-900 dark:text-white mt-5 mb-2"
        {...props}
      />
    ),
    h3: ({ ...props }) => (
...
export default function RepoReadmeModal({
  repo,
  owner,
  onClose,
}: RepoReadmeModalProps) {
  ...
  useEffect(() => {
    ...
    fetch(
      `/api/readme?owner=${encodeURIComponent(owner)}&repo=${encodeURIComponent(repo.name)}`,
    )
      .then((res) => res.json())
      .then((data) => {
        if (cancelled) return;
    ...
    >
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-slate-600 sticky top-0 bg-white dark:bg-slate-800">
          <h3 className="font-bold text-gray-900 dark:text-white truncate pr-4">
            {repo.name}
          </h3>
